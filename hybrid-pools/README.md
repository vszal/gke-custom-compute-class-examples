# Hybrid: curated manual pools on top, auto-provisioned long-tail fallback

A common production shape. Your **baseline** is one or more node pools you created
and **curated** yourself; the **long tail** is node auto-provisioning
creating generic capacity only when the baseline runs out.

- **Manual on top — control.** A pre-created pool can be backed by a
  reservation / committed use discount and can use configuration node
  auto-provisioning either doesn't create for you or
...
- **Auto-provisioning underneath — safety / obtainability.** When the curated pool
  is full or unavailable, the lower-priority rules let node auto-provisioning create
  fallback nodes so pods don't sit Pending. Best-effort capacity, not curated.
...
intent rules driving node auto-provisioning.
...
# Scale past the baseline to watch node auto-provisioning create fallback nodes:
...
back onto it from the auto-provisioned fallback nodes.
