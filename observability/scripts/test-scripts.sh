...
assert_contains "Trace output extracts RFC3339 backoff timestamp" "$TRACE_TEXT_OUT" "Backoff Until: 2026-09-17T20:45:00Z"
...
assert_eq "JSON skipped priority 0 backoffUntil matches" \
  "$(echo "$TRACE_JSON_OUT" | jq -r '.priorityAnalysis.skippedPriorities[0].backoffUntil')" "2026-09-17T20:45:00Z"
...
assert_contains "Calculated earliest backoff expiry" "$MONITOR_TEXT_OUT" "Earliest Backoff Expiry:    2026-09-17T20:42:00Z"
...
assert_eq "JSON earliestBackoffExpiration is 2026-09-17T20:42:00Z" \
  "$(echo "$MONITOR_JSON_OUT" | jq -r '.evaluation.earliestBackoffExpiration')" "2026-09-17T20:42:00Z"
